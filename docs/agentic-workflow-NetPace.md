...
[`scripts/plugin-report.sh`](../scripts/plugin-report.sh) is that missing check — a manually-run, read-only report (installs nothing, starts no model, makes no network call; the one write it can cause is context-mode's own health check creating context-mode's empty storage directories) covering four sections: `TOOLING` (expected tool → declared / installed / enabled / reachable, `pr-review-toolkit` included), `CONFIG` (unresolvable hook and statusLine paths, plus dangling or duplicated allow-entries), `HOOKS` (what is registered and what each costs per invocation), and `PERFORMANCE` (live savings counters, where a tool exposes them).
