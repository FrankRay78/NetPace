...
| `speckit-reviewissue.yml` — Speckit Review Issue | `review` label applied to an issue (author-gated) | runs the pre-spec gate unattended — see below |
...
**The pre-spec gate runs in CI too.** Step 2 of the generic sequence (`/speckit.reviewissue`) is the one step whose cost is the wait, because it carries the codebase grounding — so labelling an issue `review` runs it unattended and leaves the gap analysis waiting as a comment. The workflow reads [`.claude/commands/speckit.reviewissue.md`](../.claude/commands/speckit.reviewissue.md) and applies it, so the analysis stays single-sourced; the comment is byte-shape identical to a local run, and `/speckit.confirmissue` folds it in unchanged. The label is the state: **labelled** means a review is pending, **unlabelled with a `<!-- speckit:review -->` comment** means it is done, and **still labelled with no review comment** is the failure signal (there is no failure comment — GitHub's failed-run notification is the alert). A second run on an issue that already has a review exits without posting, because re-posting would orphan answers already written. Refining a review in place (step 6 of the command) stays local. Rationale: CIR `2026-09-07-automated-prespec-review`.
