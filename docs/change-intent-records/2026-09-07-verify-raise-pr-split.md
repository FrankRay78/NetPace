...
- Committing the fixes (step 3) keeps its "because `/raise-pr` pushes *commits*" argument, and is now more load-bearing, not less: the gap between the review fix and the push is open-ended, so an uncommitted edit has an unbounded window in which to be lost.
