...
**Intent:** Stop a recursive read of the repository escalating to a manual approval that no permission mode grants, which interrupted `/verify` step 2 when a reviewer subagent ran `grep -rn … .`.
