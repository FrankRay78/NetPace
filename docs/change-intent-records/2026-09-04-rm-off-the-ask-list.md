...
**Behaviour:**
- Given a session started in `bypassPermissions`, when `/verify`'s clean-context review (step 2) spawns the `pr-review-toolkit` reviewers, then they build, exercise and tear down a synthetic `PATH` / `NETPACE_CLAUDE_HOME` fixture with no permission prompt.
