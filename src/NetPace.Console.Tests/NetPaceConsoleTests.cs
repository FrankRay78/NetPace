...
public sealed partial class NetPaceConsoleTests
{
    ...
    [Fact]
    public async Task Should_Write_Output_To_File()
    {
        // Given
        ...
        try
        {
            ...
            // When
            var result = await app.RunAsync("--file", testFile, "--verbosity", "Minimal");

            // Then
            Assert.Equal(0, result.ExitCode);
            Assert.True(File.Exists(testFile), "Output file should be created");

            var fileContent = await File.ReadAllTextAsync(testFile);
            Assert.False(string.IsNullOrWhiteSpace(fileContent), "File should contain output");

            // Console should also show output (dual output)
            Assert.False(string.IsNullOrWhiteSpace(result.Output), "Console should also show output");
        }
        finally
        {
    ...
    [Fact]
    public async Task Should_Write_CSV_Output_To_File()
    {
        // Given
        var testFile = Path.Combine(Path.GetTempPath(), $"netpace-test-{Guid.NewGuid()}.csv");
        ...
        try
        {
            ...
            // When
            var result = await app.RunAsync("--file", testFile, "--csv");

            // Then
            Assert.Equal(0, result.ExitCode);
            Assert.True(File.Exists(testFile), "CSV output file should be created");

            var fileContent = await File.ReadAllTextAsync(testFile);
            Assert.Contains("Timestamp", fileContent, StringComparison.Ordinal);
            Assert.Contains("Latency", fileContent, StringComparison.Ordinal);
            Assert.Contains("Download", fileContent, StringComparison.Ordinal);
            Assert.Contains("Upload", fileContent, StringComparison.Ordinal);
        }
        finally
        {
            if (File.Exists(testFile))
    ...
    [Fact]
    public async Task Should_Write_JSON_Output_To_File()
    {
        // Given
        var testFile = Path.Combine(Path.GetTempPath(), $"netpace-test-{Guid.NewGuid()}.json");
        ...
        try
        {
            ...
            // When
            var result = await app.RunAsync("--file", testFile, "--json");

            // Then
            Assert.Equal(0, result.ExitCode);
            Assert.True(File.Exists(testFile), "JSON output file should be created");
    ...
    [Fact]
    public async Task Should_Write_Output_To_File_Using_Short_Flag()
    {
        // Given
        var testFile = Path.Combine(Path.GetTempPath(), $"netpace-test-{Guid.NewGuid()}.txt");
        ...
        try
        {
            ...
            // When
            var result = await app.RunAsync("-f", testFile, "--verbosity", "Minimal");

            // Then
            Assert.Equal(0, result.ExitCode);
            Assert.True(File.Exists(testFile), "Output file should be created using -f flag");
