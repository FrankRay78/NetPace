...
10. **Output result**: Print `RAISED pr=<url>` on a line of its own, naming the pull request this run opened — and print it only for a pull request this run actually opened. If no PR was opened, for any reason, print `FAILED reason=<short reason>` instead and never the `RAISED` line, even when the report quotes the URL of a PR that already exists. The two lines are the machine-readable verdict: `scripts/chain.sh` gates its last stage on them, and a bare URL in the surrounding prose is deliberately not enough, because the commonest failure here — a PR already open for this branch — reports an error whose text contains a perfectly good PR URL.

    Then, as before, if step 4 deleted a spec folder, also print `Deleted spec folder: <FEATURE_DIR>`, and print step 6's outcome in whichever of its three forms applies — `Closes #<N> — <issue title>`; `No closing keyword: <reason>` (the branch carries no issue number; `#<N>` is a pull request; `#<N>` is closed); or `Closing keyword unverified: <gh stderr>` — so the invoker can see from the output alone what merging this PR will close, if anything.
